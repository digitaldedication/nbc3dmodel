...
try {
  ...
  const rep = await page.evaluate(() => {
    ...
    scene.traverse((o) => {
      ...
      if (!out.deel1 && (o.name || '') === 'Deel_1' && o.isMesh) {
        const mats = Array.isArray(o.material) ? o.material : [o.material];
        out.deel1 = { mats: mats.map(matInfo), geomType: o.data && o.data.geometry && o.data.geometry.type };
      }
    });
    return out;
  });
  writeFileSync('scene-inspection/decor-mat.json', JSON.stringify(rep, null, 1));
  console.log('OK');
} finally {
  await browser.close();
  server.kill();
