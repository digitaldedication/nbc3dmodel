...
try {
  ...
  const rep = await page.evaluate(async () => {
    ...
    // hue = canvas-x (0..300°), helderheid = canvas-y (donker onder)
  ...
  await page.screenshot({ path: 'scene-inspection/probe-ghcal.png' });
  console.log('OK: scene-inspection/probe-ghcal.png');
} finally {
  await browser.close();
  server.kill();
}
